import { chew, cliExecute, drink, eat, effectModifier, equip, fullnessLimit, haveEffect, historicalPrice, inebrietyLimit, logprint, myFullness, myInebriety, mySpleenUse, numericModifier, print, printHtml, spleenLimit, toEffect, use, userConfirm, useSkill } from 'kolmafia';
import { $class, $effect, $item, $items, $skill, $skills, get } from 'libram/src';
...
const overrides = new Map<Effect, Override>([
  ...
  [$effect`Thanksgetting`, new Override(200, 10, 180)],
...
function normalClass(cl: Class) {
  return cl === $class`none` || cl === $class`Seal Clubber` || cl === $class`Turtle Tamer` || cl === $class`Pastamancer` || cl === $class`Sauceror` || cl === $class`Disco Bandit` || cl === $class`Accordion Thief`;
}

function safeUseItem(quantity: number, item: Item, maxPrice = 50000) {
...
// 3 turns
addOption(new Option($effect`Extra Sensory Perception`, '# gong roach itemdrop # 3 turns', historicalPrice($item`llama lama gong`)));
// 0-1 turn
addOption(new Option($effect`Snow Fortified`, '# use snow fort; camp rest # 0-1 turns', historicalPrice($item`snow fort`)));
...
// 1 turn
addOption(new Option($effect`Doing The Hustle`, '# Discotheque with 2 Disco Style', historicalPrice($item`One-day ticket to That 70s Volcano`) * 0.5));
// 0 turns (XO pocket)
addOption(new Option($effect`Tiffany's Breakfast`, '# Jar of Psychoses (artist)', historicalPrice($item`Jar of psychoses (The Pretentious Artist)`),));
// 0 turns (do day 1)
...
// ugh.
addOption(new Option($effect`High-Falutin'`, '# Grab a free drink from Gingerbread Gallery', historicalPrice($item`counterfeit city`)));
// RO stuff
addOption(new Option($effect`Octolus Gift`, '# Equip octolus-skin cloak at RO', historicalPrice($item`Octolus-skin cloak`)));
addOption(new Option($effect`Pajama Party`, '# Equip ratskin pajama pants at RO', historicalPrice($item`Ratskin pajama pants`)));
addOption(new Option($effect`Spirit of Galactic Unity`, '# Equip Spacegate scientist insignia at RO', historicalPrice($item`Spacegate scientist insignia`)));
...
for (const skill of Skill.all()) {
  if (!normalClass(skill.class)) continue;
...
selectedOptions.sort((x, y) => x.efficiency() * 10000 - approxBonus(x.effect) - (y.efficiency() * 10000 - approxBonus(y.effect)));

const passives = Skill.all().filter(skill => skill.passive && approxBonus(skill) >= 0.001 && normalClass(skill.class) && !impossiblePassives.has(skill));

const rolloverEquipment = $items`octolus-skin cloak, ratskin pajama pants, Spacegate scientist insignia`;
...
class Table {
  ...
  render() {
    const rowsHtml = this.rows.map(cells => `<tr><td>${cells.map(cell => cell.toString()).join('</td><td>')}</td></tr>`);
    return `<table border="1"><tbody>${rowsHtml.join('')}</table></tbody>`;
  }
}

export function main(argsString = '') {
  const args = argsString.trim().split(' ').map(s => s.trim()).filter(s => s.length > 0);
  if (args.length === 0) {
    print('usage: bkbuffs ( list | reminders | rollover | execute )');
  } else if (args.includes('rollover')) {
    for (const item of rolloverEquipment) {
      equip(item);
    }
    ...
    table.row(0, exactPlusItem(2 * outfitItemDrop, outfitFamiliarWeight), outfitItemDrop, outfitFamiliarWeight, 'Outfit');

    for (const passive of passives) {
      runningItemDrop += itemDrop(passive);
      runningFamiliarWeight += familiarWeight(passive);
      table.row(0, exactPlusItem(2 * runningItemDrop, runningFamiliarWeight).toFixed(), itemDrop(passive), familiarWeight(passive), passive);
    }

    for (const option of selectedOptions) {
      runningItemDrop += itemDrop(option.effect);
      runningFamiliarWeight += familiarWeight(option.effect);
      table.row(option.countNeeded() * option.getCostWithOrgan(), exactPlusItem(2 * runningItemDrop, runningFamiliarWeight).toFixed(), itemDrop(option.effect), familiarWeight(option.effect), option.effect.name);
    }
    ...
    // Thanksgetting.
    const fullnessAvailable = fullnessLimit() - myFullness() + (get('spiceMelangeUsed') ? 0 : 3) + (get('_distentionPillUsed') ? 0 : 1);
    ...
    const wishCount = selectedOptions.filter(o => typeof o.source === 'string' && o.source.startsWith('genie effect')).length;
    getItem(wishCount, $item`pocket wish`, 50000);
